'use strict';
const pending = new Map(); let serial = 0, stream, pc, stopped = false, config, after = 0, remoteSet = false, booted = false;
...
window.boot = async value => {
  ...
  try {
    stream = await navigator.mediaDevices.getUserMedia({ audio: true, video: config.video ? { facingMode: 'user' } : false });
    ...
    pc.onconnectionstatechange = () => {
      if (pc.connectionState === 'connected') { statusEl.textContent = 'Connected'; document.getElementById('hint').textContent = config.video ? 'Video call' : 'Audio call'; }
      ...
      if (pc.connectionState === 'disconnected') statusEl.textContent = 'Reconnecting…';
    };
    ...
    while (!stopped) {
      ...
      if (Date.now() - started > 90000 && pc.connectionState !== 'connected') { await hangup(); break; }
