...
function setup({ caller = true, denied = false } = {}) {
  ...
  const context={console,document:{getElementById:id=>nodes[id]??=( {textContent:'',hidden:false} )},navigator:{mediaDevices:{getUserMedia:async()=>{if(denied)throw Error('Permission denied');return stream}}},RTCPeerConnection:Peer,setTimeout:(f,ms)=>setTimeout(f,ms===1200?0:ms),clearTimeout,Date};
  context.window=context;
  context.SparkCall={send(command,json,id){const data=JSON.parse(json);commands.push({command,data});let result={ok:true};
    if(command==='poll'){polls++;result=polls===1?{call:{status:'accepted',offer:{type:'offer',sdp:'remote-offer'},answer:{type:'answer',sdp:'remote-answer'}},ice:[{id:8,candidate:{candidate:'candidate:1'}}]}:{call:{status:'ended'},ice:[]};}
